...
import io.github.pylonmc.pylon.base.content.machines.hydraulics.Excavator;
...
public class DieselChopper extends Excavator implements
        PylonTickingBlock,
    ...
    public final int dieselPerSecond = getSettings().getOrThrow("diesel-per-second", ConfigAdapter.INT);
    ...
    public static class Item extends PylonItem {
        ...
        public @NotNull List<PylonArgument> getPlaceholders() {
            return List.of(
                    ...
                    PylonArgument.of("height", UnitFormat.BLOCKS.format(depth)), // lol
                    PylonArgument.of("time-per-block", UnitFormat.SECONDS.format(blockBreakIntervalTicks / 20.0)),
                    PylonArgument.of("diesel-per-second", UnitFormat.MILLIBUCKETS_PER_SECOND.format(dieselPerSecond)),
                    PylonArgument.of("diesel-buffer", UnitFormat.MILLIBUCKETS.format(dieselBuffer))
            );
        }
    }

    public ItemStackBuilder topStack = ItemStackBuilder.of(Material.BROWN_TERRACOTTA)
    ...
    public DieselChopper(@NotNull Block block, @NotNull BlockCreateContext context) {
        super(block, context);
    ...
    @SuppressWarnings("unused")
    public DieselChopper(@NotNull Block block, @NotNull PersistentDataContainer pdc) {
        super(block, pdc);
    }
    ...
    @Override
    public void tick() {
        double dieselUsed = dieselPerSecond * getTickInterval() / 20.0;
        if (fluidAmount(BaseFluids.BIODIESEL) < dieselUsed || !tickExcavator()) {
            return;
        }
        ...
        removeFluid(BaseFluids.BIODIESEL, dieselUsed);
    }
