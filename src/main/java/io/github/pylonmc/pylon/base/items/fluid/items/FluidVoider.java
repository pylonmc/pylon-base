...
public class FluidVoider extends PylonBlock implements PylonEntityHolderBlock, PylonFluidBlock {
    ...
    public FluidVoider(@NotNull Block block, @NotNull BlockCreateContext context) {
        ...
        Preconditions.checkState(context instanceof BlockCreateContext.PlayerPlace, "Fluid voider can only be placed by a player");
        ...
        entities = Map.of(
                "main", MainDisplay.make(block, mainDisplaySize).getUuid(),
                "input", FluidConnectionInteraction.make(player, input, BlockFace.UP, (float) (mainDisplaySize / 2.0)).getUuid()
        );
    }
